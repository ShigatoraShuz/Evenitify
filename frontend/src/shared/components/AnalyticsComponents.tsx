...
export function AnalyticsMetricGrid({ metrics }: { metrics: AnalyticsMetric[] }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {metrics.map((metric) => (
        <div key={metric.label} className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          <p className="text-sm font-semibold text-slate-500">{metric.label}</p>
          <p className="mt-2 text-3xl font-semibold tracking-tighter text-slate-950">{metric.value}</p>
...
export function OperationsInsightCard({ insights }: { insights: string[] }) {
  return (
    <section className="rounded-[24px] border border-indigo-100 bg-indigo-50 p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <h3 className="text-base font-semibold tracking-tight text-indigo-950">Operations insights</h3>
      <div className="mt-3 space-y-2">
        {insights.map((insight) => (
          <p key={insight} className="text-sm leading-6 text-indigo-800">{insight}</p>
        ))}
      </div>
    </section>
  )
...
export function AdminTrendPlaceholder() {
  return (
    <section className="rounded-[24px] border border-dashed border-slate-300 bg-slate-50 p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      <h3 className="text-base font-semibold tracking-tight text-slate-950">Response trend</h3>
