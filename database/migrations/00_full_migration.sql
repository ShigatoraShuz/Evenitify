...
drop view if exists public.vendor_b2b_booking_queue;
...
-- 4. request_vendors (junction: procurement_request to vendors)
...
-- 6. Fix bookings: drop B2B-only constraint, add response_deadline
alter table public.bookings drop constraint if exists booking_type_for_event;
alter table public.bookings add column if not exists response_deadline timestamptz;

-- 7. Add denormalized FKs to contracts
alter table public.contracts
...
-- 9. RLS for service_categories
do $$ begin
  create policy "anyone can read service categories" on public.service_categories for select using (true);
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "admin can manage service categories" on public.service_categories for all using (public.is_admin());
exception when duplicate_object then null;
end $$;

-- 10. RLS for procurement_requests
do $$ begin
  create policy "organizers can read own procurement requests" on public.procurement_requests for select using (public.is_admin() or organizer_id = public.current_organizer_profile_id());
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "organizers can create procurement requests" on public.procurement_requests for insert with check (organizer_id = public.current_organizer_profile_id());
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "organizers can update own procurement requests" on public.procurement_requests for update using (organizer_id = public.current_organizer_profile_id()) with check (organizer_id = public.current_organizer_profile_id());
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "organizers can delete own procurement requests" on public.procurement_requests for delete using (organizer_id = public.current_organizer_profile_id());
exception when duplicate_object then null;
end $$;

-- 11. RLS for request_vendors
do $$ begin
  create policy "organizers and vendors can read request_vendors" on public.request_vendors for select using (public.is_admin() or exists (select 1 from public.procurement_requests pr where pr.id = request_vendors.request_id and pr.organizer_id = public.current_organizer_profile_id()) or vendor_id = public.current_vendor_profile_id());
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "organizers can manage request_vendors" on public.request_vendors for insert with check (exists (select 1 from public.procurement_requests pr where pr.id = request_vendors.request_id and pr.organizer_id = public.current_organizer_profile_id()));
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "organizers can update request_vendors" on public.request_vendors for update using (exists (select 1 from public.procurement_requests pr where pr.id = request_vendors.request_id and pr.organizer_id = public.current_organizer_profile_id())) with check (exists (select 1 from public.procurement_requests pr where pr.id = request_vendors.request_id and pr.organizer_id = public.current_organizer_profile_id()));
exception when duplicate_object then null;
end $$;

-- 12. RLS for quotes
do $$ begin
  create policy "organizers and vendors can read quotes" on public.quotes for select using (public.is_admin() or exists (select 1 from public.procurement_requests pr where pr.id = quotes.request_id and pr.organizer_id = public.current_organizer_profile_id()) or vendor_id = public.current_vendor_profile_id());
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "vendors can create quotes" on public.quotes for insert with check (vendor_id = public.current_vendor_profile_id());
exception when duplicate_object then null;
end $$;
do $$ begin
  create policy "vendors can update own quotes" on public.quotes for update using (vendor_id = public.current_vendor_profile_id()) with check (vendor_id = public.current_vendor_profile_id());
exception when duplicate_object then null;
end $$;
...
do $$ begin
  create policy "organizers can create bookings" on public.bookings for insert with check (organizer_id = public.current_organizer_profile_id() and status = 'pending');
exception when duplicate_object then null;
end $$;

-- 14. Update notification trigger — remove "B2B" label
create or replace function public.create_booking_notification()
returns trigger
language plpgsql
as $$
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title);
    insert into public.notifications (user_id, booking_id, title, message, notification_type, priority, action_url, metadata)
    values (v_vendor_user_id, new.id, 'New Booking Request', 'You have received a new booking request for event: ' || coalesce(v_event_title, 'Unknown Event'), v_notification_type, v_priority, v_action_url, v_metadata);
  end if;

  if tg_op = 'UPDATE' and old.status is distinct from new.status then
    case new.status
      when 'accepted' then v_notification_type := 'booking_status_changed'; v_priority := 'high'; v_action_url := '/organizer/portfolio?eventId=' || new.event_id; v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title, 'newStatus', new.status);
      when 'rejected' then v_notification_type := 'booking_status_changed'; v_priority := 'high'; v_action_url := '/organizer/portfolio?eventId=' || new.event_id; v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title, 'newStatus', new.status);
      when 'changes_requested' then v_notification_type := 'booking_status_changed'; v_priority := 'normal'; v_action_url := '/organizer/portfolio?eventId=' || new.event_id; v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title, 'newStatus', new.status);
      when 'confirmed' then v_notification_type := 'booking_status_changed'; v_priority := 'high'; v_action_url := '/organizer/portfolio?eventId=' || new.event_id; v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title, 'newStatus', new.status);
      when 'completed' then v_notification_type := 'booking_status_changed'; v_priority := 'normal'; v_action_url := '/organizer/portfolio?eventId=' || new.event_id; v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title, 'newStatus', new.status);
      else v_notification_type := 'booking_status_changed'; v_priority := 'normal'; v_action_url := '/organizer/portfolio?eventId=' || new.event_id; v_metadata := jsonb_build_object('bookingId', new.id, 'eventTitle', v_event_title, 'newStatus', new.status);
    end case;

    insert into public.notifications (user_id, booking_id, title, message, notification_type, priority, action_url, metadata)
    values (v_organizer_user_id, new.id, 'Booking ' || initcap(new.status::text), 'Your booking for event ' || coalesce(v_event_title, 'Unknown Event') || ' has been updated to: ' || new.status::text, v_notification_type, v_priority, v_action_url, v_metadata);
  end if;
  return new;
end;
$$;

-- 15. Update vendor_b2b_booking_queue view — remove B2B-only filter
drop view if exists public.vendor_b2b_booking_queue;
create or replace view public.vendor_b2b_booking_queue as
select
  b.id as booking_id, b.status, b.booking_type, b.requested_budget, b.notes, b.requested_at,
  le.id as event_id, le.title as event_title, le.event_date, le.venue, le.expected_guests,
  er.category as requirement_category, er.quantity as requirement_quantity,
  vp.id as vendor_profile_id, vp.business_name as vendor_business_name,
  op.id as organizer_profile_id, op.organization_name
from public.bookings b
join public.large_events le on le.id = b.event_id
join public.event_requirements er on er.id = b.requirement_id
join public.vendor_profiles vp on vp.id = b.vendor_id
...
do $$ begin
  create trigger trg_set_updated_at_procurement_requests before update on public.procurement_requests for each row execute function public.set_updated_at();
exception when duplicate_object then null;
end $$;
do $$ begin
  create trigger trg_set_updated_at_quotes before update on public.quotes for each row execute function public.set_updated_at();
exception when duplicate_object then null;
end $$;
